...
bool SkydomeShader::Initialize(ID3D11Device* device, HWND hwnd){
	...
	if (!InitializeShader(device, hwnd, SKYDOMESHADERFILEVS, SKYDOMESHADERFILEPS)){
		return false;
	}
...
bool SkydomeShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, 
...
bool SkydomeShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename){
	...
	result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "SkydomeVertexShader", "vs_4_0", 
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "SkydomePixelShader", "ps_4_0",
	...
	lightBufferDesc.ByteWidth = sizeof(SkydomeBufferType);
	...
	result = device->CreateBuffer(&lightBufferDesc, NULL, &m_skydomeBuffer);
	if(FAILED(result))
	{
		return false;
	}
					   ...
					   D3DXVECTOR4 apexColor, D3DXVECTOR4 centerColor)
{
	...
	result = deviceContext->Map(m_skydomeBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if(FAILED(result))
	{
		return false;
	}
	...
	// Unlock the constant buffer.
	deviceContext->Unmap(m_skydomeBuffer, 0);

	// Set the position of the light constant buffer in the pixel shader.
	bufferNumber = 0;

	// Finally set the light constant buffer in the pixel shader with the updated values.
	deviceContext->PSSetConstantBuffers(bufferNumber, 1, &m_skydomeBuffer);
