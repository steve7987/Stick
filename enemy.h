...
public:
	...
	bool Render(float t);
	bool Update(float t, deque<Projectile *> * projDeque, Vector targetPos);   //returns false if the enemy should be removed from the deque
...
private:
	...
	void FireShot(deque<Projectile *> * projDeque, Vector targetPos); 
	...
	float shotTimer;  //timer to countdown when to fire a shot at target
