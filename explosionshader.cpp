...
#define EXPLOSIONSHADERFILEVS L"./explosion.vs"
#define EXPLOSIONSHADERFILEPS L"./explosion.ps"
...
bool ExplosionShader::Initialize(ID3D11Device* device, HWND hwnd){
	
	if (!InitializeShader(device, hwnd, EXPLOSIONSHADERFILEVS, EXPLOSIONSHADERFILEPS)){
		return false;
	}
...
bool ExplosionShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename){
	HRESULT result;
	...
	D3D11_BUFFER_DESC explosionBufferDesc;  //new from texture shader
	...
	result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "ExplosionVertexShader", "vs_4_0", 
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "ExplosionPixelShader", "ps_4_0",
	...
	//setup explosion buffer for pixel shader.  note byteWidth needs to be multiple of 16
	explosionBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	explosionBufferDesc.ByteWidth = sizeof(ExplosionBufferType);
	explosionBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	explosionBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	explosionBufferDesc.MiscFlags = 0;
	explosionBufferDesc.StructureByteStride = 0;
	
	// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
	result = device->CreateBuffer(&explosionBufferDesc, NULL, &m_explosionBuffer);
	if(FAILED(result))
	{
		return false;
	}
...
void ExplosionShader::ShutdownShader(){
	//release the six instances that were created in init
	
	//release the light constant buffer.
	if(m_explosionBuffer)
	{
		m_explosionBuffer->Release();
		m_explosionBuffer = 0;
	}
...
void ExplosionShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename){
...
void ExplosionShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
